...
png_const_charp UTF8_BLOCK_ELEMENTS[4] = {"\xE2\x96\x88", // full block (█)
...
png_const_charp * get_block_symbols() {
    ...
    if (BLOCK_SYMBOLS == NULL) {
        ...
        if (ic == NULL) 
            fprintf(stderr, "Cannot convert block element characters to current codepage %s\n", toCs);
...
/*----------------------------------------------------------------------------*/
void read_data_cb(png_structp png_ptr, png_bytep out, size_t size) {
...
/*----------------------------------------------------------------------------*/
int get_pixel(const png_bytepp row_pointers, png_uint_32 height,
        png_uint_32 width, png_uint_32 i, png_uint_32 j) {
...
/*----------------------------------------------------------------------------*/
png_uint_32 get_horizontal_scale(
...
/*   transitions for each column                                              */
/*----------------------------------------------------------------------------*/
png_uint_32 get_vertical_scale(const png_bytepp row_pointers, png_uint_32 width,
        png_uint_32 height) {
...
/*----------------------------------------------------------------------------*/
char * png_to_txt(png_const_bytep img, size_t imgSize) {
